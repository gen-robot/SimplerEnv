...
class RoboticDiffusionTransformerModel(object):
    ...
    def __init__(
        ...
        self.policy = self.get_policy(pretrained, lora_adapter)
    ...
    def get_policy(self, pretrained, lora_adapter=None):
        ...
        if (
            ...
            _model = RDTRunner.from_pretrained(pretrained, dtype=self.dtype)

        if (
            lora_adapter is not None
            and not os.path.isfile(lora_adapter)
        ):
            print(f"[INFO] Using LORA adapter: {lora_adapter}")
            lora_config = peft.LoraConfig.from_pretrained(lora_adapter)
            _model = peft.PeftModel.from_pretrained(_model, lora_adapter, config=lora_config)
    ...
    def _format_joint_to_state(self, joints, eef_pos_rot6d=None, qvel=None):
        """
    ...
    @torch.no_grad()
    def step(self, proprio, images, text_embeds, eef_pos_rot6d=None, proprio_qvel=None, unnorm_output=True):
        """
        ...
        trajectory = self._unformat_action_to_joint(trajectory, unnorm_output).to(torch.float32)
